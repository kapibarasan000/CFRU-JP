...
static u8 GetMaxMoveType(u16 move, u8 bank, struct Pokemon* mon)
{
	...
	if (moveType == 0xFF) //No overridden type
	{
		...
		{
			moveSplit = CalcMoveSplit(bank, move, bank);
...
{
	u8 moveSplit = CalcMoveSplit(bank, baseMove, bank);
...
void LoadAndCreateEnemyShadowSprites(void)
{
	...
	LoadCompressedSpriteSheetUsingHeap((void*) 0x820D414); //gSpriteSheet_EnemyShadow

	battlerId = GetBattlerAtPosition(B_POSITION_OPPONENT_LEFT);
	gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId = CreateSprite((void*) 0x820D424, GetBattlerSpriteCoord(battlerId, BATTLER_COORD_X), GetBattlerSpriteCoord(battlerId, BATTLER_COORD_Y) + 29, 0xC8); //gSpriteTemplate_EnemyShadow
	gSprites[gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId].data[0] = battlerId;

	if (IS_DOUBLE_BATTLE)
	{
		battlerId = GetBattlerAtPosition(B_POSITION_OPPONENT_RIGHT);
		gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId = CreateSprite((void*) 0x820D424, GetBattlerSpriteCoord(battlerId, BATTLER_COORD_X), GetBattlerSpriteCoord(battlerId, BATTLER_COORD_Y) + 29, 0xC8);
		gSprites[gBattleSpritesDataPtr->healthBoxesData[battlerId].shadowSpriteId].data[0] = battlerId;
