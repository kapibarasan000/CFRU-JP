...
void HallOfFame_PrintMonInfo(struct HallofFameMon *currMon, unusedArg u8 a1, unusedArg u8 a2)
{
	...
	if (species != SPECIES_EGG)
	{
		...
		if (dexNumber != 0xFFFF)
		{
			text[0] = (dexNumber / 1000) + CHAR_0;
			text[1] = ((dexNumber %= 1000) / 100) + CHAR_0;
			text[2] = ((dexNumber %= 100) / 10) + CHAR_0;
			text[3] = (dexNumber % 10) + CHAR_0;
		}
		else
		{
			text[0] = text[1] = text[2] = text[3] = CHAR_QUESTION_MARK;
		}
		text[4] = EOS;
	...
	{
		x = 0x80 - width / 2;
		WindowPrint(0, 2, x, 1, sUnknown_083D2414, 0, text);
	}
	else
	{
		x = 0x80 - width;
