...
void atk04_critcalc(void)
{
	...
	for (u32 bankDef = 0; bankDef < gBattlersCount; ++bankDef)
	{
		...
		{
			critChance  = GetCriticalRank(gBankAttacker, gBattleMons[gBankAttacker].status2)
...
static u8 CalcPossibleCritChance(u8 bankAtk, u8 bankDef, u16 move, struct Pokemon* monAtk, struct Pokemon* monDef)
{
	...
	{
		critChance  = GetCriticalRank(bankAtk, atkStatus2)
...
static u8 GetNumHitsBasedOnMove(u16 move, u16 atkAbility, unusedArg u16 atkSpecies)
{
	...
	|| (move == MOVE_WATERSHURIKEN && atkSpecies == SPECIES_ASHGRENINJA)
		...
		numHits = 3;
	else if (CheckTableForMove(move, gTwoToFiveStrikesMoves))
	{
...
u8 TypeCalc(u16 move, u8 bankAtk, u8 bankDef, struct Pokemon* monAtk, bool8 CheckParty)
{
	...
	u8 defEffect = ITEM_EFFECT(bankDef);
	u16 atkAbility;
	u8 atkType1, atkType2, atkType3, atkTeraType;
	u8 flags = 0;
...
u8 AI_TypeCalc(u16 move, u8 bankAtk, u8 bankDef, struct Pokemon* monDef) 
{
	if (move == MOVE_STRUGGLE)
		return 0;
	...
	u16 defAbility = GetMonAbility(monDef);
	...
	if (IsMonTerastal(monDef) && monDef->teratype != TYPE_STELLAR)
	{
		defType1 = defType2 = monDef->teratype;
	}
	else if (defAbility == ABILITY_IMPOSTER && ImposterWorks(bankDef, TRUE))
	{
...
u8 AI_SpecialTypeCalc(u16 move, u8 bankAtk, u8 bankDef)
{
	...
	u8 defEffect = ITEM_EFFECT(bankDef);
	u8 atkType1, atkType2, atkType3, atkTeraType, defType1, defType2, defType3, defTeraType;
	u8 flags = 0;
	...
	atkType3 = gBattleMons[bankAtk].type3;
	atkTeraType = GetBattlerTeraType(bankAtk);
	defTeraType = GetBattlerTeraType(bankDef);
	moveType = GetMoveTypeSpecial(bankAtk, move);

	if ((atkAbility == ABILITY_PROTEAN || atkAbility == ABILITY_LIBERO) && !IsTerastal(bankAtk))
	...
	else if (gStatuses3[bankDef] & STATUS3_ILLUSION && gDisableStructs[bankDef].isFirstTurn) //Under illusion and haven't figured it out yet
	{
...
u8 VisualTypeCalc(u16 move, u8 bankAtk, u8 bankDef)
{
	...
	u8 moveType, moveEffect, defType1, defType2, defType3;
	u16 defAbility;
	...
	if (IsTerastal(bankDef) && GetTeraType(bankDef) != TYPE_STELLAR)
	{
	...
	else if (underIllusion) //Under illusion
	{
	...
	if (IsDynamaxed(bankDef) && CheckTableForMove(move, gDynamaxBannedMoves)) //These moves aren't related to type matchups, but they can still cause the move to fail and should be known to the player
	{
		flags |= MOVE_RESULT_DOESNT_AFFECT_FOE;
	}
	...
	&& ((defAbility == ABILITY_LEVITATE && NO_MOLD_BREAKERS(atkAbility, move)) || defEffect == ITEM_EFFECT_AIR_BALLOON || (gStatuses3[bankDef] & (STATUS3_LEVITATING | STATUS3_TELEKINESIS)) || underIllusion)
	&& move != MOVE_THOUSANDARROWS)
	{
		flags |= MOVE_RESULT_DOESNT_AFFECT_FOE;
	}
...
void TypeDamageModification(u16 atkAbility, u8 bankDef, u16 move, u8 moveType, u8* flags)
{
	...
	if (!IS_BLANK_TYPE(defTeraType) && defTeraType != TYPE_STELLAR)
		return TypeDamageModificationByDefTypes(atkAbility, bankDef, move, moveType, flags, defTeraType, defTeraType, defTeraType, NULL);
	else
...
TYPE_LOOP:
	...
	multiplier = ModulateMultiplier(multiplier, ModulateDmgByType(move, moveType, defType1, bankDef, atkAbility, monDef));

	if (defType1 != defType2)
		multiplier = ModulateMultiplier(multiplier, ModulateDmgByType(move, moveType, defType2, bankDef, atkAbility, monDef));

	if (!IS_BLANK_TYPE(defType3) && defType3 != defType1 && defType3 != defType2)
		multiplier = ModulateMultiplier(multiplier, ModulateDmgByType(move, moveType, defType3, bankDef, atkAbility, monDef));
	...
	if (moveType == TYPE_FIRE && gNewBS->tarShotBits & gBitTable[bankDef])
		multiplier = ModulateMultiplier(multiplier, TYPE_MUL_SUPER_EFFECTIVE);

	if (!CheckGrounding(bankDef)
	&& move == MOVE_THOUSANDARROWS
	&& (defType1 == TYPE_FLYING || defType2 == TYPE_FLYING || defType3 == TYPE_FLYING))
...
TYPE_LOOP_AI:
	multiplier = ModulateMultiplier(multiplier, ModulateDmgByType(move, moveType, defType1, 0, atkAbility, monDef));

	if (defType1 != defType2)
		multiplier = ModulateMultiplier(multiplier, ModulateDmgByType(move, moveType, defType2, 0, atkAbility, monDef));
...
static u8 ModulateDmgByType(const u16 move, const u8 moveType, const u8 defType, const u8 bankDef, u16 atkAbility, struct Pokemon* monDef)
{
	...
	if (!checkMonDef && multiplier == TYPE_MUL_NO_EFFECT)
	{
		...
		if (moveType == TYPE_PSYCHIC && defType == TYPE_DARK && (gStatuses3[bankDef] & STATUS3_MIRACLE_EYED))
			multiplier = TYPE_MUL_NORMAL; //Miracle Eye causes normal damage hits
	}
	else if (checkMonDef)
	{
...
u8 GetMoveTypeSpecialPreAbility(u16 move, u8 bankAtk, struct Pokemon* monAtk)
{
	...
	&& !AbilityCanChangeTypeAndBoost(move, ABILITY(bankAtk), gNewBS->ElectrifyTimers[bankAtk], FALSE))
		return TYPE_ELECTRIC;
...
u8 GetMonExceptionMoveType(struct Pokemon* mon, u16 move)
{
	...
	switch (move) {
		...
		case MOVE_REVELATIONDANCE:
			if (gMain.inBattle && IsMonTerastal(mon))
...
//Load attacker Data
	...
	else //Load from bank
	{
		...
		data->moveSplit = CalcMoveSplit(bankAtk, move, bankDef);
...
//Stat Buffs - Attacker
	...
	&& (data->atkBuff != 6 || data->spAtkBuff != 6))
	{
...
static u16 GetBasePower(struct DamageCalc* data)
{
	...
	switch (move) {
		...
		case MOVE_TERABLAST:
			if (!useMonAtk && GetBattlerTeraType(bankAtk) == TYPE_STELLAR)
				power = 100;
			break;
...
static u16 AdjustBasePower(struct DamageCalc* data, u16 power)
{
	...
	switch(data->atkAbility) {
		...
		//1.3x Boost
			...
			|| (useMonAtk && gBattleMoves[move].flags & FLAG_MAKES_CONTACT))
...
static u32 ScreensWeakenDamage(u32 damage, bool8 screensUp, u16 atkAbility, u8 bankDef)
{
	...
	&& gCritMultiplier <= BASE_CRIT_MULTIPLIER && atkAbility != ABILITY_INFILTRATOR)
	{
