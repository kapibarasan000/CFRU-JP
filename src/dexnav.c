...
static void Task_ManageDexNavHUD(u8 taskId)
{
	...
	if (sDexNavHudPtr->totalProximity < 1)
	{
		...
		u16 dexNum = SpeciesToNationalPokedexNum(species);
		if (dexNum < 999 && gDexNavSearchLevels[dexNum] < 255)
...
bool8 InitDexNavHUD(u16 species, u8 environment, bool8 detectorMode)
{
	...
	u16 dexNum = SpeciesToNationalPokedexNum(species);
	u8 searchLevel = dexNum < 999 ? gDexNavSearchLevels[dexNum] : 0;
...
static void PrintDexNavMessage(u8 messageId)
{
	...
	WindowPrint(WIN_TEXTBOX, 2, 4, 0, &sBlackText, GetPlayerTextSpeedDelay(), gStringVar4);
	RunTextPrinters();
...
static void PrintGUIAreaName(void)
{
	...
	WindowPrint(WIN_MAP_NAME, 1, 2, 2, &sWhiteText, 0, gStringVar4);
...
static void PrintGUIChainLength(void)
{
	...
	WindowPrint(WIN_CHAIN_LENGTH, 1, x, 2, colour, 0, gStringVar4);
