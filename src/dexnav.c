...
#define sDexNavHudPtr (*((struct DexnavHudData**) 0x203DFB0))
#define sDexNavGuiPtr (*((struct DexNavGuiData**) 0x203DFB0))
...
static bool8 ShakingGrass(u8 environment, u8 xSize, u8 ySize, bool8 smallScan)
{
	if (DexNavPickTile(environment, xSize, ySize, smallScan))
	{
		...
		{
			if (gSprites[i].callback == (void*) 0x080DDD05)
			{
...
static void DexNavLoadAreaNamesAndInstructions(void)
{
	...
	WindowPrint(WIN_LAND, 0, 5, 5, &sDexNav_WhiteText, 0, landText);
	WindowPrint(WIN_WATER, 0, 4, 5, &sDexNav_WhiteText, 0, waterText);

	//Print Location Name
	GetMapName(gStringVar4, GetCurrentRegionMapSectionId(), 0);
	WindowPrint(WIN_MAP_NAME, 1, 2, 1, &sDexNav_WhiteText, 0, gStringVar4);

	//Print Instructions
	WindowPrint(WIN_INSTRUCTIONS, 0, 5, 2, &sDexNav_WhiteText, 0, gText_DexNavInstructions);
