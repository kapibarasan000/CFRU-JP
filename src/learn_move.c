...
#else
	#define gLevelUpLearnsets ((struct LevelUpMove**) *((u32*) 0x804346C)) //extern const struct LevelUpMove* const gLevelUpLearnsets[];
#endif

//#define gMoveToLearn (*((u16*) 0x2024022))
#define sLearningMoveTableID (*((u8*) 0x2023F88))
#define sMoveRelearnerStruct ((struct MoveRelearner*) 0x203AA2C)
...
move_t RandomizeMove(u16 move)
{
	...
	if (newMove == MOVE_NONE || newMove == MOVE_STRUGGLE)
		return newMove + 1;
...
struct MoveRelearner
{
    ...
    u8 unk_1D;
    u8 unk_1E;
	struct ListMenuItem menuItems[MAX_MOVE_REMINDER_MOVES];	//0x20 - 0x168
	u16 moves[MAX_MOVE_REMINDER_MOVES];
	u8 filler[0x28]; //Recycle old string buffer section
	...
	const u8 listMenuNames[MAX_MOVE_REMINDER_MOVES][MOVE_NAME_LENGTH - 6];
};

#define gMoveRelearnerStruct (*((struct MoveRelearner**) 0x203AA2C))
#define gText_MoveRelearnerAskTeach (u8*) 0x83C5A9C
