...
static u32 CalcPredictedDamageForCounterMoves(u16 move, u8 bankAtk, u8 bankDef)
{
	...
	if (predictedMove != MOVE_NONE && SPLIT(predictedMove) != SPLIT_STATUS && !MoveBlockedBySubstitute(predictedMove, bankDef, bankAtk))
	{
		...
		switch (move) {
			case MOVE_COUNTER:
				if (CalcMoveSplit(bankDef, predictedMove, bankAtk) == SPLIT_PHYSICAL)
					predictedDamage *= 2;
				else
					predictedDamage = 0;
				break;
			case MOVE_MIRRORCOAT:
				if (CalcMoveSplit(bankDef, predictedMove, bankAtk) == SPLIT_SPECIAL)
					predictedDamage *= 2;
				else
					predictedDamage = 0;
				break;
...
bool8 PhysicalMoveInMoveset(u8 bank)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (CalcMoveSplit(bank, move, bank) == SPLIT_PHYSICAL
			&& gBattleMoves[move].power != 0
			&& gBattleMoves[move].effect != EFFECT_COUNTER)
				return TRUE;
		}
...
bool8 SpecialMoveInMoveset(u8 bank)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (CalcMoveSplit(bank, move, bank) == SPLIT_SPECIAL
			&& gBattleMoves[move].power != 0
			&& gBattleMoves[move].effect != EFFECT_MIRROR_COAT)
				return TRUE;
		}
...
bool8 StatusMoveInMoveset(u8 bank)
{
	...
	for (int i = 0; i < MAX_MON_MOVES; ++i)
	{
		...
		if (!(gBitTable[i] & moveLimitations))
		{
			if (CalcMoveSplit(bank, move, bank) == SPLIT_STATUS)
				return TRUE;
		}
	}
