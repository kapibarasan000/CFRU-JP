...
const s8 gAbilityRatings[ABILITIES_COUNT] =
{
	...
	[ABILITY_EMBODYASPECT_CORNERSTONEMASK] = 3,
...
static u8 TryActivateTerrainAbility(u8 terrain, u8 anim, u8 bank, bool8 special);
...
u8 AbilityBattleEffects(u8 caseID, u8 bank, u16 ability, u16 special, u16 moveArg)
{
	...
	case ABILITYEFFECT_ON_SWITCHIN: // 0;
		...
		case ABILITY_ORICHALCUMPULSE:
			...
			{
				gBattleStringLoader = gText_OrichalcumPulseActivate;
		...
		case ABILITY_SCHOOLING:
			if (!(gBattleMons[bank].status2 & STATUS2_TRANSFORMED))
			{
				...
				{
					DoFormChange(bank, SPECIES_WISHIWASHI_S, TRUE, TRUE, FALSE);
				...
				&& (gBattleMons[bank].level < 20 ||
					gBattleMons[bank].hp <= (gBattleMons[bank].maxHP / 4)))
				{
					DoFormChange(bank, SPECIES_WISHIWASHI, TRUE, TRUE, FALSE);
		...
		case ABILITY_SHIELDSDOWN:
			if (!(gBattleMons[bank].status2 & STATUS2_TRANSFORMED))
			{
				...
				{
					DoFormChange(bank, umodsi(GetBankPartyData(bank)->personality, 7), TRUE, TRUE, FALSE); //Get Minior Colour
				...
				&& gBattleMons[bank].hp > (gBattleMons[bank].maxHP / 2))
				{
					DoFormChange(bank, SPECIES_MINIOR_SHIELD, TRUE, TRUE, FALSE);
		...
		case ABILITY_ELECTRICSURGE:
			effect = TryActivateTerrainAbility(ELECTRIC_TERRAIN, B_ANIM_ELECTRIC_SURGE, bank, FALSE);
			break;

		case ABILITY_GRASSYSURGE:
			effect = TryActivateTerrainAbility(GRASSY_TERRAIN, B_ANIM_GRASSY_SURGE, bank, FALSE);
			break;

		case ABILITY_MISTYSURGE:
			effect = TryActivateTerrainAbility(MISTY_TERRAIN, B_ANIM_MISTY_SURGE, bank, FALSE);
			break;

		case ABILITY_PSYCHICSURGE:
			effect = TryActivateTerrainAbility(PSYCHIC_TERRAIN, B_ANIM_PSYCHIC_SURGE, bank, FALSE);
			break;

		case ABILITY_HADRONENGINE:
			if (gTerrainType == ELECTRIC_TERRAIN)
			{
				gBattleStringLoader = gText_HadronEngineActivate;
				BattleScriptPushCursorAndCallback(BattleScript_SwitchInAbilityMsg);
				effect++;
			}
			else
				effect = TryActivateTerrainAbility(ELECTRIC_TERRAIN, B_ANIM_ELECTRIC_SURGE, bank, TRUE);
			break;
		...
		case ABILITY_EMBODYASPECT_CORNERSTONEMASK:
			if (STAT_STAGE(bank, STAT_STAGE_DEF) < STAT_STAGE_MAX)
			{
				gBankAttacker = bank;
				gBattleScripting.statChanger = STAT_STAGE_DEF | INCREASE_1;
		...
		case ABILITY_EMBODYASPECT_WELLSPRINGMASK:
		...
		case ABILITY_EMBODYASPECT_TEALMASK:
	...
	case ABILITYEFFECT_ENDTURN: // 1
		...
		case ABILITYEFFECT_CONTACT: //After being hit by a move. Not necessarilly contact.
			...
			case ABILITY_WANDERINGSPIRIT:
				...
				&& gBankAttacker != bank
				&& *GetAbilityLocation(gBankAttacker) != gLastUsedAbility
				&& ITEM_EFFECT(gBankAttacker) != ITEM_EFFECT_PROTECTIVE_PADS
				&& CheckContact(move, gBankAttacker, bank))
				{
					BattleScriptPushCursor();
			...
			case ABILITY_GULPMISSILE:
				...
				&& gBankAttacker != bank)
				{
					...
					if (effect)
					{
						DoFormChange(bank, SPECIES_CRAMORANT, TRUE, TRUE, FALSE); //Revert back
						BattleScriptPushCursor();
	...
	if (gTerrainType != terrain)
	{
		...
		switch (terrain) {
			case ELECTRIC_TERRAIN:
				if (special)
					gBattleStringLoader = gText_HadronEngineSetString;
				else
...
static void TextIntoAbilityPopUp(void *dest, u8 *windowTileData, s32 arg2, bool32 arg3)
{
	...
	if (arg2 > 0)
	{
		...
		{
			if (arg3)
				CpuCopy32(windowTileData + 8, dest + 8, 24);
			else
...
static void PrintAbilityOnAbilityPopUp(u32 ability, u8 spriteId1, u8 spriteId2)
{
	...
	PrintOnAbilityPopUp(abilityName,
						(void*)(OBJ_VRAM0) + (gSprites[spriteId1].oam.tileNum * 32) + 512,
						(void*)(OBJ_VRAM0) + (gSprites[spriteId2].oam.tileNum * 32) + 512,
						8, 0,
						0,
						7, 9, 1);
}
...
static const u16 sOverwrittenPixelsTable[][2] =
{
	...
	{PIXEL_COORDS_TO_OFFSET(0, 13), 3},
	{PIXEL_COORDS_TO_OFFSET(0, 14), 3},
	{PIXEL_COORDS_TO_OFFSET(0, 15), 3},

	{PIXEL_COORDS_TO_OFFSET(8, 15), 8},
	{PIXEL_COORDS_TO_OFFSET(16, 15), 8},
	{PIXEL_COORDS_TO_OFFSET(24, 15), 8},
	{PIXEL_COORDS_TO_OFFSET(32, 15), 8},
	{PIXEL_COORDS_TO_OFFSET(40, 15), 8},
	{PIXEL_COORDS_TO_OFFSET(48, 15), 8},
	{PIXEL_COORDS_TO_OFFSET(56, 15), 8},
	...
	{PIXEL_COORDS_TO_OFFSET(0, 27), 5},
	{PIXEL_COORDS_TO_OFFSET(0, 28), 5},
	{PIXEL_COORDS_TO_OFFSET(0, 29), 8},
	{PIXEL_COORDS_TO_OFFSET(0, 30), 8},
	{PIXEL_COORDS_TO_OFFSET(0, 31), 8},

	{PIXEL_COORDS_TO_OFFSET(8, 29), 8},
	{PIXEL_COORDS_TO_OFFSET(16, 29), 8},
	{PIXEL_COORDS_TO_OFFSET(24, 29), 8},
	{PIXEL_COORDS_TO_OFFSET(32, 29), 8},
	{PIXEL_COORDS_TO_OFFSET(40, 29), 8},
	{PIXEL_COORDS_TO_OFFSET(48, 29), 8},
	{PIXEL_COORDS_TO_OFFSET(56, 29), 8},

	{PIXEL_COORDS_TO_OFFSET(8, 30), 8},
	{PIXEL_COORDS_TO_OFFSET(16, 30), 8},
	{PIXEL_COORDS_TO_OFFSET(24, 30), 8},
	{PIXEL_COORDS_TO_OFFSET(32, 30), 8},
	{PIXEL_COORDS_TO_OFFSET(40, 30), 8},
	{PIXEL_COORDS_TO_OFFSET(48, 30), 8},
	{PIXEL_COORDS_TO_OFFSET(56, 30), 8},

	{PIXEL_COORDS_TO_OFFSET(8, 31), 8},
	{PIXEL_COORDS_TO_OFFSET(16, 31), 8},
	{PIXEL_COORDS_TO_OFFSET(24, 31), 8},
	{PIXEL_COORDS_TO_OFFSET(32, 31), 8},
	{PIXEL_COORDS_TO_OFFSET(40, 31), 8},
	{PIXEL_COORDS_TO_OFFSET(48, 31), 8},
	{PIXEL_COORDS_TO_OFFSET(56, 31), 8},
...
//Second Row Of Image
	...
	{PIXEL_COORDS_TO_OFFSET(0, 61), 8},
	{PIXEL_COORDS_TO_OFFSET(0, 62), 8},
	{PIXEL_COORDS_TO_OFFSET(0, 63), 8},
	{PIXEL_COORDS_TO_OFFSET(8, 61), 8},
	{PIXEL_COORDS_TO_OFFSET(8, 62), 8},
	{PIXEL_COORDS_TO_OFFSET(8, 63), 8},
	{PIXEL_COORDS_TO_OFFSET(16, 61), 8},
	{PIXEL_COORDS_TO_OFFSET(16, 62), 8},
	{PIXEL_COORDS_TO_OFFSET(16, 63), 8},
};
