	...
	struct FossilTable
	{
		...
		struct FossilTable gFossilImageTable[] =
		{
			[0] = //Kabutops (originally index 0x8D, now 0x0)
				{
					.data = (struct SpriteSheet*) 	0x83A68C4,
					.palette = (u16*) 				0x83A68A4,
				},
			[1] =
				{ //Aerodactyl (originally index 0x8e, now 0x1)
					.data = (struct SpriteSheet*) 	0x83A68D4,
					.palette = (u16*) 				0x83A6084,
				},
			[2] =
...
void sp011_RibbonSetterCleaner(void) {
	u16 mon = Var8004;
	u16 ribbon = Var8005;
	u32* ribbonArray = (u32*) &(((u8*) 0x2024230)[mon * sizeof(struct Pokemon)]);
...
///////////////////////////////////////////////////////////////////////////////////
	...
	#define gSafariBallNumber (*((u16*) 0x0203990C))
#else
	#define gSafariBallNumber (*((u8*) 0x0203990C))
#endif
...
#define gTMCasePositionStruct ((u8*) 0x203B080)
...
#define FossilTemplate ((struct SpriteTemplate*) 0x83A586C)
bool8 sp18B_ShowFossilImage(void) {
	...
	if (Var8004 == 0x8D)
	{
		LoadSpriteSheets((void*) 0x83A68C4);
		pal = AddPalRef(5, Var8004);	// dynamic OW pals
		if (pal == 0xFF)
			LoadPalette((void*) 0x83A68A4, 0x1d0, 0x20);
		else
			DoLoadSpritePalette((void*) 0x83A68A4, pal*16);
	}
	else if (Var8004 == 0x8E)
	{
		LoadSpriteSheets((void*) 0x83A68D4);
		pal = AddPalRef(5, Var8004);	// dynamic OW pals
		if (pal == 0xFF)
			LoadPalette((void*) 0x83A6084, 0x1d0, 0x20);
		else
			DoLoadSpritePalette((void*) 0x83A6084, pal*16);
	}
	else
		return FALSE;
...
void HallOfFame_PrintMonInfo(struct HallofFameMon *currMon, unusedArg u8 a1, unusedArg u8 a2) {
	...
	if (currMon->species != SPECIES_EGG) {
		...
		stringPtr[0] = EOS;
		WindowPrint(0, 2, 0x10, 1, sUnknown_083D2414, 0, text);
	}
	...
	if (currMon->species == SPECIES_EGG) {
		...
		WindowPrint(0, 2, width, 1, sUnknown_083D2414, 0, text);
		CopyWindowToVram(0, 3);
	} else {
		...
		stringPtr[0] = EOS;
		WindowPrint(0, 2, 0x80, 1, sUnknown_083D2414, 0, text);

		stringPtr = StringCopy(text, gText_Level);
		ConvertIntToDecimalStringN(stringPtr, currMon->lvl, STR_CONV_MODE_LEFT_ALIGN, 3);
		WindowPrint(0, 2, 0x20, 0x11, sUnknown_083D2414, 0, text);

		stringPtr = StringCopy(text, gText_IDNumber);
		ConvertIntToDecimalStringN(stringPtr, (u16)(currMon->tid), STR_CONV_MODE_LEADING_ZEROS, 5);
		WindowPrint(0, 2, 0x60, 0x11, sUnknown_083D2414, 0, text);
...
#define sPlayerNamingScreenTemplate (const struct NamingScreenTemplate*) 0x83A8540
#define sPCBoxNamingTemplate (const struct NamingScreenTemplate*) 0x83A854C
#define sMonNamingScreenTemplate (const struct NamingScreenTemplate*) 0x83A8558
#define sRivalNamingScreenTemplate (const struct NamingScreenTemplate*) 0x83A8564
...
#define sHeaderBoxWindowId (*((u8*) 0x20399A0)) //Steal coin box Id
...
u8* sScriptStringVars[12] =
{
	...
	gStringVarD,	//FD 0D
#else
	(u8*) 0x83DD1EA, //FD 07 = RUBY
	(u8*) 0x83DD1F2, //FD 08 = MAGMA
	(u8*) 0x83DD1EE, //FD 09 = AQUA
	(u8*) 0x83DD1FB, //FD 0A = MAXIE
	(u8*) 0x83DD1F6, //FD 0B = ARCHIE
	(u8*) 0x83DD206, //FD 0C = GROUDON
	(u8*) 0x83DD200, //FD 0D = KYOGRE
#endif
};
