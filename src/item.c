...
#define gTMHMLearnsets ((TM_HM_T*) *((u32*) 0x80432B4))
#define gTutorLearnsets ((ExpandedTutor_T*) *((u32*) 0x8121420))
...
{
	gTasks[taskId].func = (void*) 0x8125F21;
}
...
void LoadTMNameWithNo(u8* dst, u16 itemId)
{
	...
	StringCopy(gStringVar4, (void*) 0x83DDBCB);

	if (tmNum > NUM_TMS)
	{
		// HM
		u8 hmNum = tmNum - NUM_TMS;
		StringAppend(gStringVar4, (void*) 0x83DDBD3);
		StringAppend(gStringVar4, (void*) 0x83DD856);
		if (NUM_HMS < 10)
	...
	StringAppend(gStringVar4, (void*) 0x83DDBCF);

	if (StringLength(gMoveNames[ItemIdToBattleMoveId(itemId)]) == (MOVE_NAME_LENGTH - 7) && tmNum > NUM_TMS)
...
void CheckTmPurchase(u16 item, u8 taskId)
{
	CopyItemName(item, &gStringVar1[0]);
		...
		if (GetPocketByItemId(item) == POCKET_TM_HM)
		{
			...
			BuyMenuDisplayMessage(taskId, (void*) 0x83DDC1B, Task_BuyHowManyDialogueInit);
	#else
		BuyMenuDisplayMessage(taskId, (void*) 0x83DDC1B, Task_BuyHowManyDialogueInit);
	#endif
}
...
bool8 CheckBuyableTm(u16 item, u8 taskId)
{
		...
		else
		{
			...
			{
				BuyMenuDisplayMessage(taskId, (void*) 0x83DDCCE, BuyMenuReturnToItemList);
				return TRUE;
			}
		}
	#else
...
{	
	...
	if (GetPocketByItemId(tItemId) == POCKET_TM_CASE)
		BuyMenuDisplayMessage(taskId, (void*) 0x83DDC75, Task_ReturnToSellListAfterTmPurchase);
	else
	#endif
		BuyMenuDisplayMessage(taskId, (void*) 0x83DDC75, (TaskFunc) (0x809B8FC | 1)); //BuyMenuSubtractMoney
}
#undef tItemId
...
//Bag Expansion////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#define gBagPockets ((struct BagPockets*) 0x20397D8)

#define sBagRegularItems ((struct ItemSlot*) 0x203BA98)
...
#define sPokeDudeItemBackupPtr2 (*((struct ItemSlot***) 0x203B090))
...
#define sCurrentBagPocket (*((u8*) 0x203AC7A))
...
#define gMenuText_Use (const u8*) 0x83DD7E6
#define gMenuText_Toss (const u8*) 0x83DD7EA
#define gMenuText_Register (const u8*) 0x83DD7EE
#define gMenuText_Give (const u8*) 0x83DD7F3
#define gText_Cancel2 (const u8*) 0x83DD801
#define gMenuText_Check (const u8*) 0x83DD823
#define gMenuText_Open (const u8*) 0x83DD829
#define gMenuText_Walk (const u8*) 0x83DD7FD
#define gMenuText_Deselect (const u8*) 0x83DD82D
#define gText_EmptyString2 (const u8*) 0x83DD809

#define ItemMenu_UseOutOfBattle (void*) 0x810A74D
#define ItemMenu_Toss (void*) 0x810A7BD
#define ItemMenu_Register (void*) 0x810AAFD
#define ItemMenu_Give (void*) 0x810ABA5
#define ItemMenu_Cancel (void*) 0x810ADD9
#define ItemMenu_UseInBattle (void*) 0x810AE21
#define ItemMenu_Register (void*) 0x810AAFD
...
void Task_PrintItemDescriptionOnKeyRelease(u8 taskId)
{
		...
		|| (!JOY_HELD(DPAD_DOWN) && !JOY_HELD(DPAD_UP))) //Stopped scrolling
	{
		...
		BagPrintTextOnWindow(1, 2, description, 0, 3, 1, 3, 0, 0);
		DestroyTask(taskId);
	}
}
...
void PrintItemDescriptionOnMessageWindow(u16 itemIndex)
{
	...
	{
		sItemDescriptionPocket = gBagMenuState.pocket + 1; //Update description's pocket number
		description = (void*) 0x83DD8E3; //gText_CloseBag;
		BagPrintTextOnWindow(1, 2, description, 0, 3, 1, 3, 0, 0); //Print description right away
		TryDestroyItemDescriptionTask(); //Task no longer needed
	}
}
