...
{
	{gStatusConditionString_Poison, (const u8*) 0x83C47B4},
	{gStatusConditionString_Sleep, (const u8*) 0x83C47B0},
	{gStatusConditionString_Paralysis, (const u8*) 0x83C47BB},
	{gStatusConditionString_Burn, (const u8*) 0x83C47B7},
	{gStatusConditionString_Ice, (const u8*) 0x83C47BE},
	{gStatusConditionString_Confusion, (const u8*) 0x83C47C2},
	{gStatusConditionString_Love, (const u8*) 0x83C47C7},
...
const struct BattleMsgData** gStringInfo = ((const struct BattleMsgData**) 0x20399AC);
		...
		if (*src == B_BUFF_PLACEHOLDER_BEGIN) //0xFD
		{
			...
			case B_TXT_TRAINER1_CLASS: // trainer class name
				if (gTrainerBattleOpponent_A == 0x400) //Lol Secret Bases
					...
					toCpy = gTrainerClassNames[sub_8043E08()];
...
void BufferMoveNameBattle(u16 move, u8* dst)
{
	...
	else if (IsAnyMaxMove(move))
	{
		dst[0] = PC_DA,
		dst[1] = PC_II,
		StringCopy(&dst[2], gMoveNames[move]);
	}
	else
