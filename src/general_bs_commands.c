...
static bool8 DoesBankNegateDamage(u8 bank, unusedArg u16 move)
{
	...
	if (ability == ABILITY_ICEFACE && species == SPECIES_EISCUE && SPLIT(move) == SPLIT_PHYSICAL && !IS_TRANSFORMED(bank))
		return TRUE;
	#endif
...
void atk0C_datahpupdate(void)
{
	...
	if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) || (gHitMarker & HITMARKER_NON_ATTACK_DMG))
	{
		...
		&& !IS_TRANSFORMED(gActiveBattler))
		{
			...
			if (CalcMoveSplit(gBankAttacker, gCurrentMove, gActiveBattler) == SPLIT_PHYSICAL)
			{
			...
			else if (CalcMoveSplit(gBankAttacker, gCurrentMove, gActiveBattler) == SPLIT_SPECIAL)
			{
...
bool8 CanLastMoveNotBeEncored(u8 bank)
{
	...
	return lastUsedMove == MOVE_STRUGGLE
		...
		|| CheckTableForMove(lastUsedMove, gMovesThatCallOtherMoves)
...
void atkBE_rapidspinfree(void)
{
	...
	if (gCurrentMove == MOVE_RAPIDSPIN)
	{
		...
		else
		{
			...
			&& STAT_CAN_RISE(bankAtk, STAT_STAGE_SPEED)) //From Gen 8
			{
