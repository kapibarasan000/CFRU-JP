...
void atk09_attackanimation(void)
{
	...
	else
	{
		if (gNewBS->ParentalBondOn == 1)
		{
			...
			&&  (GetBaseMoveTarget(gCurrentMove, gBankAttacker) & (MOVE_TARGET_BOTH | MOVE_TARGET_FOES_AND_ALLY | MOVE_TARGET_DEPENDS)
		...
		if (!(resultFlags & MOVE_RESULT_NO_EFFECT))
		{
			...
			if (move == MOVE_EXPANDINGFORCE && GetMoveTarget(move, FALSE) & MOVE_TARGET_BOTH)
				gBattleScripting.animTurn = 1; //Play doubles animation
...
void atk0C_datahpupdate(void)
{
	...
	if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) || (gHitMarker & HITMARKER_NON_ATTACK_DMG))
	{
		...
		&& !IS_TRANSFORMED(gActiveBattler))
		{
			...
			if (CalcMoveSplit(gActiveBattler, gCurrentMove, gActiveBattler) == SPLIT_PHYSICAL)
			{
			...
			else if (CalcMoveSplit(gActiveBattler, gCurrentMove, gActiveBattler) == SPLIT_SPECIAL)
			{
...
void atk7E_setreflect(void) {
	if (gSideStatuses[SIDE(gBankAttacker)] & SIDE_STATUS_REFLECT) {
		...
		gBattleStringLoader = gText_ReflectRaisedStat;
		gBattleCommunication[MULTISTRING_CHOOSER] = 1;
	}
	gBattlescriptCurrInstr++;
}
...
void atk92_setlightscreen(void)
{
	...
	else
	{
		...
		gBattleStringLoader = gText_LightScreenRaisedStat;
		gBattleCommunication[MULTISTRING_CHOOSER] = 1;
	}
	gBattlescriptCurrInstr++;
}
...
void atkB9_magnitudedamagecalculation(void)
{
	...
	{
		gBattlescriptCurrInstr = (u8*) 0x81BA8EC;
		return;
	}
