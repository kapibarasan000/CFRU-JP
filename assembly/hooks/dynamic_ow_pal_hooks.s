...
LoadPalFossilImage:
	...
	lsl r1, #4
	ldr r2, =(0x800851C+1) @ LoadOBJPalette
	bl CallByR2
...
DeleteOBJFreeTiles:
	...
	mov r5, r0
	ldr r0, [r0,#0x14]
	ldr r1, =0x81F11F4 @ template for empty OBJ
	cmp r0, r1
...
SetPalNPC4:
	mov r0, r4
	ldr r1, =0x8007578+1 @ LoadAnimFrame, needed for instant refreshing
	bl CallByR1
	ldrh r0, [r7,#2]
	bl FindOrLoadNPCPalette
	lsl r1, r0, #4
...
RefreshFameCheckerNPCs:
	...
	mov r2, #0x10
	mov r3, #0
	ldr r4, =0x8044838+1 @ FadePalette, color palettes black if fading in
	bl CallByR4
	pop {r4}

RefreshFameCheckerNPCs_DoNotColor:
	ldr r1, =0x8006724+1 @ RefreshOBJs
	bl CallByR1
	cmp r5, #1
